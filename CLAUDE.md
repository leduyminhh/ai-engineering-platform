...
- Content spans `core` (principles + `git-workflow`, 1 skill) and 5 plugins under `plugins/`: `backend` (8 skills) and `frontend` (6 skills) are **published** (offered by the wizard, shipped in the npm package); `engineering` (6 skills), `data` (4 skills), and `ops` (3 skills) are **draft** — they build/validate and install via explicit `--plugin`, but are not offered by default nor shipped (see `plugins/_published.json`). There is no mandatory pipeline; every skill is `pipeline: false` and invoked on demand.
